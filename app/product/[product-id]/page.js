...
export default async function Product({ params }) { 
  ...
  console.log("🆔 ID recibido en Product:", productId);
  ...
  return (
    <main className="flex flex-row pt-10 flex-wrap h-[calc(100vh-4rem)] mt-17 overflow-hidden">
      ...
      <div className="w-3/5 h-4/5 p-7 pt-0 bg-color-sand rounded-lg flex flex-col justify-between">
        ...
        <div>
          {/* 🔹 Título con sombra */}
          <h2 className="text-4xl font-extrabold text-brownn mb-3 drop-shadow-lg">{data.name}</h2>
          <h3 className="text-xl text-green font-semibold mb-4">{data.brand}</h3>
          ...
          <div className="mt-6 flex items-center flex-wrap bg-green/50 text-lg rounded-md px-3 py-1 font-semibold w-8/10">
            <p className="text-lg font-semibold text-ligth-sand drop-shadow-lg">Product Description</p>
